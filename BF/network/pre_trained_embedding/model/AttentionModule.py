...
def glorot_uniform(shape, scale=1.0, mode='fan_avg', dtype=tf.float32):
    return tf.keras.initializers.VarianceScaling(scale=scale, mode=mode, distribution='uniform')(shape, dtype=dtype)

def random_normal(shape, std, dtype=tf.float32):
...
class Attention(tf.Module):
    def __init__(self, config, name="attention", global_config=None):
        ...
        with self.name_scope:
            ...
            self.q_weights = tf.Variable(glorot_uniform(shape, dtype=self.DTYPE), name='query_w', dtype=self.DTYPE)
            self.k_weights = tf.Variable(glorot_uniform(shape, dtype=self.DTYPE), name='key_w'  , dtype=self.DTYPE)
            self.v_weights = tf.Variable(glorot_uniform(shape, dtype=self.DTYPE), name='value_w', dtype=self.DTYPE)

            o_shape = [self.num_head, self.value_dim, self.output_dim]
            self.o_weights = tf.Variable(glorot_uniform(o_shape,   dtype=self.DTYPE), name='output_w', dtype=self.DTYPE)
...
class GlobalAttention(tf.Module):
    def __init__(self, config, name="attention", global_config=None):
        super(GlobalAttention, self).__init__(name=name)
        with self.name_scope:
            self.gc = global_config
            self.config = config
            self.key_dim = self.config["key_dim"]
            ...
            self.gating = self.config["gating"]
            shape = [self.input_dim, self.num_head, self.key_dim]
            self.q_weights = tf.Variable(glorot_uniform(shape), name='query_w')
            shape = [self.input_dim, self.key_dim]
            self.k_weights = tf.Variable(glorot_uniform(shape), name='key_w')
            shape = [self.input_dim, self.value_dim]
            self.v_weights = tf.Variable(glorot_uniform(shape), name='value_w')


            o_shape = [self.num_head, self.value_dim, self.output_dim]
            self.o_weights = tf.Variable(glorot_uniform(o_shape), name='output_w')
            self.o_bias = tf.Variable(tf.zeros((self.output_dim,)), name='output_b')
            if self.gating:
                g_shape = [self.input_dim, self.num_head, self.value_dim]
                self.gating_weights = tf.Variable(tf.zeros(g_shape), name='gating_w')
                self.gating_bias = tf.Variable(tf.ones([self.num_head, self.value_dim]), name='gating_b')

    def __call__(self, q_data, m_data, q_mask=None):
        v = tnp.einsum('bka,ac->bkc', m_data, self.v_weights)
        
        q_avg = tnp.mean(q_data, axis=1)

        q = tnp.einsum('ba,ahc->bhc', q_avg, self.q_weights) * self.key_dim ** (-0.5)
        k = tnp.einsum('bka,ac->bkc', m_data, self.k_weights)
        logits = tnp.einsum('bhc,bkc->bhk', q, k)
        weights = tf.nn.softmax(logits)
        weighted_avg = tnp.einsum('bhk,bkc->bhc', weights, v)

        if self.gating:
            gate_values = tnp.einsum('bqc, chv->bqhv', q_data, self.gating_weights)
            gate_values = tf.nn.sigmoid(gate_values + self.gating_bias)
            weighted_avg = weighted_avg[:, None] * gate_values
            output = tnp.einsum('bqhc,hco->bqo', weighted_avg, self.o_weights) + self.o_bias
        else:
            output = tnp.einsum('bhc,hco->bo', weighted_avg, self.o_weights) + self.o_bias
            output = output[:, None]
...
class MSARowAttentionWithPairBias(tf.Module):
    def __init__(self, config, name="msa_row_attention_with_pair_bias", global_config=None, dtype=tf.float32):
        ...
        with self.name_scope:
            ...
            init_factor = 1./np.sqrt(float(self.pair_input_dim))
            self.weights = tf.Variable(random_normal(shape, init_factor, dtype=self.DTYPE), name='feat_2d_weights', dtype=self.DTYPE)
...
class OuterProductMean(tf.Module):
    ...
    def __init__(self, config, name='outer_product_mean', global_config=None):
        ...
        with self.name_scope:
            ...
            shape = [self.num_input_channel, self.num_outer_channel]
            scale = np.sqrt(1./self.num_input_channel)
            self.DTYPE = tf.float32 #DTYPE
            ...
            self.output_w = tf.Variable(glorot_uniform(o_shape, scale=2., mode='fan_in', dtype=self.DTYPE), name="output_w", dtype=self.DTYPE)
